...
package body GAda.Core is
   ...
   -- Configuration
   Console_Encoding : String := "utf8" ; -- xterm encoding, not necessarily the locale encoding
   Emacs_Encoding   : String := "utf8" ; -- emacs encoding, not necessarily the locale encoding
   Latin1_Encoding   : String := "latin1" ; -- emacs encoding, not necessarily the locale encoding

   -- We check the status of the main program frequently (check for exception or termination)
   Program_Check_Interval : Guint32 := 100 ; -- Milliseconds

   Max_Line_Length : Gint := 1500 ; -- Maximum number of characters on a line.
   Max_Lines       : Gint := 4000 ; -- Maximum number of lines
   ...
   -- Status bar, below
   Status_HBox     : Gtk.Box.Gtk_Box;
   Status_Label1   : Gtk.Label.Gtk_Label ;
   Status_Label2   : Gtk.Label.Gtk_Label ;
   ...
   function CleanUTF (Arg : String) return Glib.UTF8_String is
      Valid : Boolean ;
      Pos : Natural ;
   begin
      Glib.Unicode.UTF8_Validate(Arg, Valid, Pos) ;
      if Valid then
	 --Ada.Text_IO.Put_Line("Valid.") ;
	 return Arg ;
      else
	 --Ada.Text_IO.Put_Line("Not Valid.") ;
	 return Glib.Convert.Convert(Arg, "UTF-8", Latin1_Encoding) ;
      end if ;
   end CleanUTF ;
...
--                                                   Style => Pango_Style_Italic,
      ...
      -- Status Label
      Gtk.Box.Gtk_New_Hbox(Status_HBox);
      Gtk.Label.Gtk_New(Status_Label1, "Execution time :") ;
      Gtk.Label.Set_Padding(Status_Label1, 5, 5) ;
      Gtk.Label.Modify_Font(Status_Label1, Label1_Font) ;
      Gtk.Box.Pack_Start(Status_HBox, Status_Label1, False, False) ;

      Gtk.Label.Gtk_New(Status_Label2, "--") ;
      Gtk.Label.Set_Padding(Status_Label2, 5, 5) ;
      Gtk.Label.Modify_Font(Status_Label2, Label1_Font) ;
      Gtk.Box.Pack_Start(Status_HBox, Status_Label2, False, False) ;


      Gtk.Window.Add(Main_Window, Main_Vbox);
      Gtk.Box.Pack_Start(Main_Vbox, Frame);
      Gtk.Box.Pack_End(Main_Vbox, Status_HBox, False, false);
   ...
   -- Numbers of chars in the buffer at the beginning of the current line.
   Size_At_Bol : Gint := 0 ;

   -- Current line is blocked (no more append is possible).
   Line_Is_Blocked : Boolean := False ;
   Buffer_Is_Blocked : Boolean := False ;
   Slow_Down : Boolean := False ;

   procedure Reset_Line is
   begin
      Slow_Down := False ;
      Line_Is_Blocked := False ;
      Size_At_Bol := Gtk.Text_Buffer.Get_Char_Count(Buffer) ;
   end Reset_Line ;

   procedure Append(M : String ; Force : Boolean := False) is
   begin
      if Force or ((not Line_Is_Blocked) and (not Buffer_Is_blocked)) then

         if Gtk.Text_Buffer.Get_Line_Count(Buffer) > Max_Lines then
            Buffer_Is_Blocked := True ;
            Gtk.Text_Buffer.Get_End_Iter(Buffer, IterEnd) ;
            Gtk.Text_Buffer.Insert(Buffer, IterEnd, "---too much text, stopping output---") ;
         else

            -- If the current line is too long, cut
            if Gtk.Text_Buffer.Get_Char_Count(Buffer) - Size_At_Bol > Max_Line_Length then
               Line_Is_Blocked := True ;

               Gtk.Text_Buffer.Get_End_Iter(Buffer, IterEnd) ;
               Gtk.Text_Buffer.Insert(Buffer, IterEnd, "[...]") ;
            else
   ...
   type MyDuration is delta 0.1 digits 8;

   procedure Update_Time_Label is
      Current_Time : Time := Clock ;
      Dur : MyDuration := MyDuration(Current_Time - Starting_Time) ;
   begin
      Gtk.Label.Set_Text(Status_Label2, MyDuration'Image(Dur) & " s") ;
   end Update_Time_Label ;
   ...
   begin
      ...
      if not Main_Task_Finished then
         ...
         Ada.Text_IO.Put_Line("[[-- Le programme a deja quitte'. --]]") ;	 
      end if ;
   end;
